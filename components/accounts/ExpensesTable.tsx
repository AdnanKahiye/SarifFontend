...
import DepositForm, { CreateExpenseRequest } from "./ExpensesFormModal";
import ConfirmDeleteModal from "../ui/Model/ConfirmDeleteModal";
import { AccountService } from "@/lib/account";
import toast from "react-hot-toast";
import { Loader2, ChevronLeft, ChevronRight } from "lucide-react";
import DepositFormModal  from "./ExpensesFormModal";
...
export default function ExpensesTable() {
  ...
  const [data, setData] = useState<ExpensesDto[]>([]);
  ...
  const [selectedItem, setSelectedItem] = useState<ExpensesDto | null>(null);
...
const formatMoney = (amount?: number, currencyId?: number) => {
  ...
  const handleEdit = (item: ExpensesDto) => {
    ...
    <div className="bg-[#f3f3f9] dark:bg-gray-900 min-h-screen p-4 sm:p-6 font-sans text-[#495057]">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
          <h2 className="text-[15px] font-bold uppercase">Expense List</h2>
          <div className="text-[13px] font-medium text-gray-500">Account &gt; Expenses</div>
        </div>
...
<DepositFormModal
  ...
  onSubmit={async (data: CreateExpenseRequest) => {
    try {
