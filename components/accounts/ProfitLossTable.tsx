...
// Helitaanka taariikhda hadda
const now = new Date();

// Helitaanka 1-da bisha hadda
const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1)
  .toISOString()
  .split('T')[0];

// Helitaanka taariikhda maanta
const today = now.toISOString().split('T')[0];

// State-ka oo loo beddelay default-ka cusub
...
const [toDate, setToDate] = useState(today);
  ...
  useEffect(() => {
    loadData(1, fromDate, toDate); // Bilowgii wuxuu load-gareynayaa maanta
  }, [loadData]); 
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white dark:bg-gray-800 p-4 rounded border border-gray-200 dark:border-gray-700 shadow-sm">
            <p className="text-[12px] text-gray-500 uppercase">Total Revenue</p>
            <h3 className="text-xl font-bold text-green-600">{data?.totalRevenue.toFixed(2) || "0.00"}</h3>
          </div>
          <div className="bg-white dark:bg-gray-800 p-4 rounded border border-gray-200 dark:border-gray-700 shadow-sm">
            <p className="text-[12px] text-gray-500 uppercase">Total Expense</p>
            <h3 className="text-xl font-bold text-red-600">{data?.totalExpense.toFixed(2) || "0.00"}</h3>
          </div>
          <div className="bg-white dark:bg-gray-800 p-4 rounded border border-gray-200 dark:border-gray-700 shadow-sm">
            <p className="text-[12px] text-gray-500 uppercase">Profit</p>
            <h3 className={`text-xl font-bold ${(data?.profit ?? 0) < 0 ? "text-red-600" : "text-green-600"}`}>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-sm overflow-hidden">
          ...
          <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex flex-wrap items-center gap-4">
            ...
            <button onClick={() => loadData(1, fromDate, toDate)} className="bg-[#299cdb] text-white px-6 py-2 rounded text-[13px] ml-auto hover:bg-[#2386bd] transition-all">Show Report</button>
          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                {data?.details.data && data.details.data.length > 0 ? data.details.data.map((item, idx) => (
                  <tr key={idx} className="text-[13px] hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-700">
          ...
          <div className="p-4 flex items-center justify-between border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800">
            ...
            <div className="flex items-center gap-1">
              <button disabled={currentPage === 1 || loading} onClick={() => setCurrentPage(p => p - 1)} className="p-1.5 border border-gray-200 dark:border-gray-700 rounded disabled:opacity-40 hover:bg-gray-50 dark:hover:bg-gray-700"><ChevronLeft size={16} /></button>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button key={page} onClick={() => setCurrentPage(page)} className={`px-3 py-1.5 rounded text-[13px] transition-all ${currentPage === page ? "bg-[#405189] text-white shadow-md font-bold" : "border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400"}`}>{page}</button>
              ))}
              <button disabled={currentPage >= totalPages || loading} onClick={() => setCurrentPage(p => p + 1)} className="p-1.5 border border-gray-200 dark:border-gray-700 rounded disabled:opacity-40 hover:bg-gray-50 dark:hover:bg-gray-700"><ChevronRight size={16} /></button>
            </div>
          </div>
        </div>
      </div>
