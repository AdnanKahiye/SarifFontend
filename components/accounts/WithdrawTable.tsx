...
import DepositForm, { CreateWithdrawRequest } from "./WithdrawFormModal";
...
export default function WithdrawTable() {
  ...
  const [selectedItem, setSelectedItem] = useState<WithdrawDto | null>(null);
...
const formatMoney = (amount?: number, currencyId?: number) => {
  ...
  const loadData = useCallback(async (page: number, useFilters: boolean = true) => {
    setLoading(true);
    try {
      const res = await AccountService.getWithdrawals(
        page, 
        itemsPerPage, 
  ...
  const handleEdit = (item: WithdrawDto) => {
    ...
    <div className="bg-[#f3f3f9] dark:bg-gray-900 min-h-screen p-4 sm:p-6 font-sans text-[#495057]">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
          <h2 className="text-[15px] font-bold uppercase">Withdraw List</h2>
          <div className="text-[13px] font-medium text-gray-500">Account &gt; Withdraws</div>
        </div>
    ...
    return (
      <span className={`px-2 py-1 rounded text-xs font-medium ${status.class}`}>
        {status.text}
          ...
          {/* DATE */}
          <td className="p-3">
            {new Date(item.withdrawnAt).toLocaleDateString("en-US", {
              month: "2-digit",
              day: "2-digit",
              year: "2-digit",
            })}
...
<DepositFormModal
  ...
  onSubmit={async (data: CreateWithdrawRequest) => {
    try {
      if (isEdit && selectedItem) {
        // UPDATE
       await AccountService.updateWithdrawal(selectedItem.id, data);
        toast.success("Transaction updated");
      } else {
        // ADD
        await AccountService.createWithdraw(data);
